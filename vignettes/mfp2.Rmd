...
$\alpha_1$ and $\alpha_2$ by AIC-ad or BIC-ad.
...
significance levels (p-value), adapted Akaike information criterion (AIC-ad), and adapted Bayesian
information criterion (BIC-ad). It also provides functions for prediction and
...
The `mfp2` package offers alternative approaches to variable and function selection by utilizing information criteria. Within the MFP framework, each covariate is evaluated univariately while adjusting for the other covariates through an overarching back-fitting algorithm. This algorithm iteratively assesses each covariate and selects the model (null, linear, FP1, FP2) with the minimum AIC-ad (criterion set to aic) or BIC-ad (criterion set to bic). Here, AIC and BIC are referred to as AIC-ad and BIC-ad to distinguish them from the original AIC and BIC. However, in the program, we retain the original abbreviations AIC and BIC.
...
allows users to specify whether they want to use AIC-ad or BIC-ad criteria for both
variable and function selection. If AIC-ad or BIC-ad is selected as the criterion, the
nominal significance levels set through the `select` and `alpha` arguments will 
be ignored. For details on using AIC-ad and BIC-ad in variable and function selection,
...
an alternative approach is to directly use adapted AIC or BIC, defined below.

\setlength{\jot}{0.5pt}
\begin{align*}
AIC-ad & = -2 \log(L) + 2k \\
BIC-ad & = -2 \log(L) + \log(n) \times k,
\end{align*}
...
and $n$ is the sample size or the number of observations in the dataset. AIC-ad and
BIC-ad consider both model fit and complexity, with lower values indicating
...
number of parameters for each model type is used in calculating the AIC-ad and BIC-ad.
The model with the smallest AIC-ad or BIC-ad is then selected for $z$.
...
```{r, eval = TRUE, fig.cap = "\\label{fig:fig4}Art data. Illustration on how to use terms_seq argument in fracplot.The first option is to use the original data that was used to fit the model (terms_seq = "data"). The second option is to generate a sequence of equidistant new data points using the range of the original data for the variable of interest (terms_seq ="equidistant"). In the original x5 values, there are no values between 62.21 and 235. Consequently, the FP2 function, when applied to the original dataset, exhibits a linear trend instead of a curve in the right tail region."}
...
y <- as.numeric(20*sigmoid(x) + rnorm(n, mean = 0, sd = 0.8))
```
...
```{r,echo=FALSE, fig.cap="\\label{fig:fig6}The selected function using the ACD transformation is FP1(1,1)(left panel) while the usual MFP selected FP2(3,3)(right panel). See text for more details", fig.width=8, fig.height=4}
figurex
```
