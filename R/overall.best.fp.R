...
overall.best.fp <- function(x, y, xi, allpowers,df, weights,offset,family,
 ...
 N <- dim(x)[1L]
  ...
  if(df==1){
    fits <- overall.best.linear(x = x,y = y, xi = xi,allpowers = allpowers,
    ...
    pvalue = fits$pvalues
    fit = list(overall.best.fn = as.numeric(best.fp.power),dev.all = dev.all,aic.all =  aic.all,
      ...
      if(acdx[xi]){
        ...
        bfpa <- bestfpa(y = y,x = x, xi = xi, allpowers = allpowers,powers = powers,family = family,
        ...
        if(criterion=="pvalue"){
          if(ftest){best.fp.power <- bestfuns[[5]][[index.bestmodel]]}else{best.fp.power <- bestfuns[[1]][[index.bestmodel]]}
        }else{
        ...
        if(verbose){
          if(criterion=="pvalue"){
            if(ftest){
              fprint2(namex = xi, dev.all =dev.roy.all, df.res = df.all,dev.diff = dev.diff,f =fstatistic,
      ...
      bfp1 <- bestfp1(y = y,x = x, xi = xi, allpowers = allpowers,powers = powers,family = family,
                      method = method, weights = weights,offset = offset,
                      strata = strata,control = control,rownames = rownames,
                      nocenter = nocenter, acdx = acdx)
      ...
      if(degree==1){
        ...
        if(criterion=="AIC"){
          ...
          if(pvalue[1]>select){index.bestmodel <- 1}else{index.bestmodel <- ifelse(pvalue[2]>alpha,2,3)}
        }
      }else{
        # Here we fit other fpm models where m can be 2, 3, and so on
        fpx <- bestfpm.all(y=y,x = x, xi=xi, allpowers=allpowers, powers=powers,family = family,
                           method = method, weights = weights, offset = offset,
                           strata=strata,control = control,rownames = rownames,
        ...
        if(criterion=="AIC"){
          ...
          names(pvalue) <- names(dev.diff) <- c("Null","Linear", if(dd!=0){paste0("FP",seq_len(dd))})
          # Variable selection
          if(pvalue[1]>select){
            index.bestmodel <- 1
            # function selection
      ...
      if(index.bestmodel==1){
        ...
        best.fp.power = switch(criterion,"pvalue" = if(ftest){bestfp1x[5]}else{bestfp1x[1]},
                               "AIC" = bestfp1x[2],"BIC" = bestfp1x[3])
      }else{ # best FPm function
        best.fp.power = switch(criterion,
                               "pvalue" = if(ftest){bestfp.dev.roy[[index.bestmodel-2]]}else{bestfp.dev[[index.bestmodel-2]]}, # bestfp.dev = c(FP1, FP2,...) SO subtract 2 to get correct index in bestfp.dev
      ...
      if(verbose){
        if(criterion=="pvalue"){
            best.function1 <- if(degree==1){list(lin = 1, fpm = if(ftest){bestfp1x[5]}else{bestfp1x[1]})}else{append(if(ftest){bestfp.dev.roy}else{bestfp.dev}, list(1), 0)}
            if(ftest){
              fprint2(namex = xi, dev.all =dev.roy.all,df.res = df.all, dev.diff = dev.diff,f=fstatistic,
                      df.den = df.all, pvalues = pvalue, best.function = best.function1,
                      index.bestmodel = index.bestmodel,acd = F)
            }else{
            fprint1(namex = xi, dev.all = dev.all,dev.diff = dev.diff,
                    pvalues = pvalue,index.bestmodel = index.bestmodel,
                    best.function =best.function1, acd = F)
            }
          # AIC and BIC display
        }else{
          best.function.aic <- if(degree==1){list(lin = 1, fpm = bestfp1x[2])}else{append(bestfp.aic, list(1), 0)}
          best.function.bic <- if(degree==1){list(lin = 1, fpm = bestfp1x[3])}else{append(bestfp.bic, list(1), 0)}
          switch (criterion,
      ...
      fit = list(overall.best.fn = as.numeric(best.fp.power),dev.all = if(ftest){dev.roy.all}else{dev.all},
                 aic.all =  aic.all,
                 bic.all = bic.all,index.bestmodel = index.bestmodel,
                 dev.diff = if(criterion=="pvalue"){dev.diff}else{NA},
                 pvalues = if(criterion=="pvalue"){pvalue}else{NA})
  }
  return(fit)
}
