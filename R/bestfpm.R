...
bestfpm <- function(y,x, xi, allpowers, powers, family,weights, offset,strata,
  ...
  m <- degree
  df1 <- adjustment.data(x = x, xi = xi, allpowers = allpowers,
  ...
  mt <- list(dev.best.fpm = devs[which.min(devs)],# Deviance of the best FPm function
             ...
             dev.roy.best.fpm = devs.royston[which.min(devs.royston)])# Deviance of the best FPm function based on royston formula
