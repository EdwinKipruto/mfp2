...
linear model. In this section, we will demonstrate how to fit this model using
our package. We will use the prostate cancer data (Stamey et al., 1989) included
with our package. The dataset contains seven predictors (six continuous variables
and one binary variable) and a continuous outcome variable (log prostate-specific antigen (lpsa)) of 97 patients with prostate cancer.

Our aim is to determine whether non-linear functional relationships exist in the
data.

Load the `prostate` dataset from the `mfp2` package

```{r out.lines = 10}
data("prostate_ca")
head(prostate_ca)
```
The command loads a dataframe from the R data archive since the .
