...
bestfp1 <- function(y,x, xi, allpowers, powers, family, method, weights,
  ...
  df1 <- adjustment.data(x = x, xi=xi, allpowers=allpowers, df = 2, acdx=acdx,
                         powers = powers)
  ...
  #=============================================================================
    ...
    fn.bestfp1 <- setNames(c(s1[which.min(devs)],
                             s1[which.min(aic)],
                             s1[which.min(bic)],
                             s1[which.min(sse)],
  ...
  # return
  outx <- list(dev.all = dev.all, aic.all = aic.all, bic.all = bic.all,
