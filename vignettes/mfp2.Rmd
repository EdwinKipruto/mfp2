...
## Linear Regression
This section aims to provide users with a comprehensive understanding of the `mfp2` package using classical linear regression models. We will present a concise overview of the package's key functions and resulting outputs, delving into each function in detail to highlight their specific use.
...
in our package. The dataset contains seven predictors (six continuous variables (age, pgg45, cavol, weight, bph and cp)
and one binary (svi) variable and a continuous outcome variable (log prostate-specific antigen (lpsa)) of 97 patients with prostate cancer. Our aim is to determine whether non-linear functional relationships exist between the predictors and
...
```
The command loads a data frame from the **R** data archive since the `mfp2` package 
is already loaded. We create a matrix `x` and a numeric vector `y` from the data frame to be used by the default interface of `mfp2()` function. 
