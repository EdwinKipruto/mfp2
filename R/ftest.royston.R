...
ftest.royston <- function(dev,resid.df,n,acd){
  if(acd){
    ...
    for(i in 1:4){
          stats <- fstats.stata(dev.reduced =dev[i], dev.full = dev[5], d1 = df[i],
                                d2 =resid.df[5], n = n)
    ...
    # e). M3 vs M5
      stats2 <- fstats.stata(dev.reduced =dev[6], dev.full = dev[4], d1 = df[5],
                             d2 =resid.df[4], n = n)
    ...
    k <- seq(m-1, 1)
    df <- c(2*m, 2*m-1, if(m>=2){2*(k)}) # same as 2*(m-k) where k = seq(1,m-1)
    ...
    for(i in 1:(nn-1)){
      stats <- fstats.stata(dev.reduced =dev[i], dev.full = dev[nn], d1 = df[i],
                            d2 =resid.df[nn], n = n)
      pvalues[i] <- stats$pval
      fstatistic[i] <-stats$fstatistic
      dev.diff[i] <- stats$dev.diff
