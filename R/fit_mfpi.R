...
#'     for the retained interaction variables.}
#'   \item{\code{all_fitted_functions}}{Named list (one element per
#'     \code{cont_var}) of fitted-function matrices using the pre-specified form,
#'     regardless of whether the variable was selected.}
...
fit_mfpi <- function(x, y, family, family_string, weights, offset, cycles,
  ...
  #'     one element per variable, using the pre-specified form.}
  ...
  evaluate_interactions <- function(y, processed_data, selected_vars,
    ...
    all_fitted_functions   <- vector("list", n_vars)   # one per variable
    ...
    for (var_name in cont_vars) {
      ...
      if (criterion %in% c("aic", "bic")) {
        ...
        if (!is.null(m)) {
          ...
          ic_col   <- if (criterion == "aic") "dAIC" else "dBIC"
          main_ic  <- m[[main_col]][1L]
          int_ic   <- m[[int_col]][1L]
          if (is.finite(main_ic) && is.finite(int_ic)) {
            dIC          <- main_ic - int_ic
            best_type    <- active_type
            best_fit     <- candidate_full_fits[[active_type]]
            best_metric  <- m
            best_score   <- dIC
            best_metric[[ic_col]] <- dIC
          }
        }
      }
      ...
      if (!adjusting_pvals && !is.null(best_fit)) {
        ...
        if (passes_threshold) {
          ...
          if (verbose && criterion %in% c("aic", "bic")) {
            ic_col <- if (criterion == "aic") "dAIC" else "dBIC"
            ic_val <- if (ic_col %in% names(best_metric)) best_metric[[ic_col]][1L] else NA_real_
            cat(sprintf(
              "        >> Selected  (d%s = %s)\n",
              toupper(criterion),
      ...
      # Update all_metrics_list with the annotated best_metric so that
      # all_model_metrics carries dAIC/dBIC for AIC/BIC criterion.
    ...
    if (verbose && criterion %in% c("aic", "bic") && !adjusting_pvals) {
      ...
      for (vn in cont_vars) {
        ...
        if (is.null(w$fit) || is.null(w$metric)) {
          ...
          ic_val     <- if (ic_col %in% names(w$metric)) w$metric[[ic_col]][1L] else NA_real_
