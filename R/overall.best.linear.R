...
overall.best.linear <- function(x,y, xi,allpowers,df ,weights,offset, control,
                                family,criterion, select,alpha,keep,powers,method,
                                strata,ftest,rownames, nocenter, verbose, acdx){
#  Set df = 1 in the adjustment.data() because linearity is assumed and fpdata
# would be original x of interest (untransformed)
  xadjv = adjustment.data(x = x, xi = xi, allpowers = allpowers, df = 1,
                          powers = powers, acdx = acdx) # degree, s and scale does not play any role here
  ...
  modelnull <- model.fit(x = xadjv$adjdata, y = y,family = family,weights = weights,
  ...
  modellinear <- model.fit(x =xx , y = y,family = family,weights = weights,
                           offset = offset, method = method, strata = strata,
                           control= control,rownames = rownames,
                           nocenter = nocenter)
  ...
  dev.roy.lin <- dev.gaussian(RSS = sse.linear, weights = weights, n = N)
  # Residual degrees of freedom. It includes scale parameter. for consistency
  ...
  if(criterion=="AIC"){
    ...
    if(ftest){
      stats <- fstats.stata(dev.reduced =dev.roy.all[1], dev.full = dev.roy.all[2], d1 = 1,
                            d2 =df.all[2], n = N)
      pvalue <- stats$pval
  ...
  if(verbose){
    if(criterion=="pvalue"){
      if(ftest){
      fprint2(namex = xi, dev.all =dev.roy.all,df.res = df.all, df.den =df.all, dev.diff = dev.diff,f =fstatistic,
              pvalues =pvalue,best.function =list(1), index.bestmodel = index.bestmodel,acd = acdx[xi]) #acdx[xi]
      }else{
        fprint1(namex = xi, dev.all =dev.all, dev.diff = dev.diff,
                pvalues = pvalue,
                index.bestmodel = index.bestmodel,
                best.function =list(1), acd = acdx[xi]) #acdx[xi]
      }
    }else{
      switch (criterion,
              "AIC" = fprint3(xi, gic = aic.all, keep = keep, best.function = list(1), acd = F),
              "BIC" = fprint3(xi, gic = bic.all, keep = keep, best.function = list(1), acd = F))
    }

  }

  fit = list(overall.best.fn = as.numeric(ifelse(index.bestmodel==1, NA, 1)),
             dev.all = if(ftest){dev.roy.all}else{dev.all},aic.all =  aic.all,
