...
bestfpa <- function(y,x, xi, allpowers, powers, family, method, weights,
  ...
  df1 <- adjustment.data(x = x, xi = xi, allpowers = allpowers, df = 4,
  ...
  #=============================================================================
  fitnull<- model.fit(x = adjdata, y = y, family = family, method = method,
  ...
  fit.lin.xi<- model.fit(x = xk, y = y, family = family, method = method,
                      weights = weights, offset = offset,strata = strata,
  ...
  fit.lin.axi<- model.fit(x =xkk , y = y, family = family, method = method,
                         weights = weights, offset = offset,strata = strata,
                         control = control,rownames =rownames,
                         nocenter = nocenter)
  # Total number of parameters in the fitted model.
  ...
  fit.fp1.xi<- bestfp1(y = y,x = x, xi = xi, allpowers = allpowers,
  ...
  fit.fp1.axi<- bestfp1(y = y,x = xx, xi = xi, allpowers = allpowers,
                       powers = powers, family = family, method = method,
                       weights = weights, offset = offset,strata = strata,
                       control = control,rownames = rownames,
                       nocenter = nocenter, acdx = acdxi)
  ...
  for(i in seq_len(nv)){
    ...
    fit1<- model.fit(x = xout, y = y, family = family, method = method,
                     weights = weights, offset = offset, strata = strata,
                     control = control,rownames = rownames,
                     nocenter = nocenter)
  ...
  s = df1$powers
  fn.bestfp1 <- list(dev = s[which.min(devs),],aic = s[which.min(aic),],
  ...
  for(i in 1:5){
    all.funs[[i]]<- list(null= c(NA,NA), linxi = c(1,NA), FP1xi = c(bestfpxi[i],NA),
         FP1axi =c(NA,bestfpaxi[1]),  FP1xiaxi =fn.bestfp1[[i]], linaxi = c(NA,1))
  }
  outs<- list(dev.all = dev.all, aic.all = aic.all,bic.all = bic.all,
