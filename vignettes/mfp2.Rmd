...
```{r, eval = TRUE, fig.cap = "\\label{fig:fig4}Art data. Illustration on how to use terms_seq argument in fracplot.The first option is to use the original data that was used to fit the model. The second option is to generate a sequence of equidistant new data points using the range of the original data for the variable of interest. In the original x5 values, there are no values between 62.21 and 235. Consequently, the FP2 function, when applied to the original dataset, exhibits a linear trend instead of a curve in the right tail region."}
