...
In R, the `model.matrix()` function automatically generates dummy variables for factors. For ordinal variables, users should assign an appropriate contrast matrix to preserve order information. The `mfp2` package provides the `contr.cumulative()` function for creating cumulative contrasts. Nominal variables can remain as standard factors.  

Before fitting a model, convert the variable to a factor using `factor()` and set `ordered = TRUE` for ordinal variables. Levels can be specified explicitly; otherwise, the first level is the reference.
...
fit_formula <- mfp2(y ~ fp(x1) + x2 + fp(x3) + x4 + fp(x5) + fp(x6) + fp(x7) + x8 + x9 + fp(x10), data = art, verbose = FALSE)
