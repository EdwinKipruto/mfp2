...
#'@param power The user-supplied FP power. Default is 1 (linear)
...
#'@param center Specification of centering for the independent variables using
...
fracgen <- function(x, power=1, scale = NULL, center = F, shift = NULL){
  ...
  if(nx<=2){
    ...
    x <- x + shift
    # scale x
    if(is.null(scale)){scale <- scalefn(x)}
    x <- x/scale
