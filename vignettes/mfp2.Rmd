...
Section 2 serves as an introduction to the `mfp2` package. It covers the
...
such as logistic (Subsection 2.4), Poisson (Subsection 2.5), and Cox (Subsection
2.6), are briefly explained.
...
variable (subsection 3.1) and may help mitigate the influence of extreme
covariate values on a selected function (subsection 3.2).
...
displaying eight FP1 powers and the middle panel depicting a subset of FP2
powers.
...
modeling in practice are given in Sauerbrei et al. (2007b) and in Royston and
...
function. This is because binary or factor variables may not undergo FP 
...
will fit an MFP model with a single covariate ($x5$).
...
illustrated below, which produced Figure 3.
...
interpretability of regression coefficients and may help reduce the influence of
extreme covariate values on a selected function (see Figure 5 for an example).
