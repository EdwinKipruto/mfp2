...
This vignette describes basic usage of `mfp2` in R. There are additional vignettes that should be useful:

This vignette provides a comprehensive overview of the basic usage of `mfp2` in R. Additionally, there are additional vignettes available that re designed to further enhance your understanding on the mfp2 package.

* ["Introduction to Multivariable Fractional Polynomial"](insert link here for 
mfp) provides an overview of multivariable fractional polynomials.
...
and one binary variable) and a continuous outcome variable (log prostate-specific antigen (lpsa)) of 97 patients with prostate cancer. Our aim is to determine whether non-linear functional relationships exist in the
data.
...
```
The command loads a dataframe from the R data archive since the `mfp2` package is already loaded. We create a matrix `x` and a numeric vector `y` from the dataframe. The default interface of `mfp2()` requires a matrix of predictors and a numeric vector of response `y` for continuous outcomes.


We fit the Gaussian linear model using the  default interface of `mfp2()`.
...
allows the `mfp2` to utilize methods and functions specific to `glm` and `lm`. Various methods are defined for extracting components from the `mfp2` object, such as coef, print, summary, fracplot, and predict. These methods allow users to access different components of the mfp2 object, including coefficient estimates,
summary statistics, plotting functionalities, and predictions.

We demonstrate how to fit the same model using the formula interface. In this 
case, we will use the `fp()` function, which requires the continuous variable 
as an argument. For more comprehensive information on the fp() function, please refer to section xx

```{r, eval=FALSE}
fit <- mfp2(lpsa ~ fp(age) + svi + fp(pgg45) + fp(cavol) + fp(weight) + fp(bph) +
              fp(cp), data = prostate)
```

The main distinction between the `mfp2()` and `glm()` functions in R is the inclusion of the `fp()` function within the formula used by `mfp2()`. The presence of the `fp()` function indicates that the variables included within it should undergo fractional polynomial (FP) transformation, provided that the degree of freedom is not equal to 1.

It's worth noting that the variable `svi` is a binary variable, hence not passed 
to the `fp()` function. Since FP transformation is not applicable to binary or
factor variables, it is excluded from the transformation process. However, if a binary variable is passed to the `fp()` function, the program will automatically set the degrees of freedom to 1, treating the variable as linear. Factor variables
will return an error when passed to `fp()` function.
