...
varorder <- function(x, y, weights, offset, family, method, strata,control,
  ...
  if(family!="cox"){
    ...
    for(i in 1:ns){
      ...
      fit.reduced <- glm.fit(x =cbind(rep(1,n),x[,-i, drop=FALSE]),y = y,
    ...
    fit.full <- coxfit(x = x, y = y, strata = strata, weights = weights,
                       offset = offset, control = control,method = method,
                       rownames = rownames, nocenter=nocenter)    # full model
    ...
    for(i in 1:ns){
      # remove one variable at a time and fit the reduced model
      fit.reduced <- coxfit(x = x[,-i, drop=FALSE], y = y, strata = strata,
