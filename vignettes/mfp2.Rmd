...
```{r terms_seq, eval = TRUE, fig.cap = "Art data. Illustration on how to use terms_seq argument in fracplot", fig.show = 'hold'}
...
known to reduce recurrence rates and is forced into the model.


We use mfp to build a model from the
...
```{r out.lines = 103, R.options = list(width = 80)}
# load gbsg data
data("gbsg")
gbsg <- create_dummy_variables(gbsg, var_ordinal = "grade", drop_variables = FALSE)

head(gbsg, 10)

# matrix x
x <- as.matrix(gbsg[,c(2:9,12,13)])

# outcome y
y <- survival::Surv(gbsg$rectime, gbsg$censrec)

# fit mfp and keep hormon in the model
mfp2(x, y, family = "cox", keep = "hormon", verbose = FALSE)
```
...
fit <- mfp2(y ~ fp(x, acdx = TRUE), data = datax, verbose = FALSE)
...
The selected function is FP1(1,1) which is equivalent to $\beta_0 + \beta_1x^1 +\beta_2a^1$. The function is shown in Figure 5.
```{r,fig.cap="Art data. Sigmoid relationship between x and outcome",fig.align='center', echo=FALSE, out.width= '45%',out.height='50%'}
