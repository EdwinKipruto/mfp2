...
#Extracts the adjustment variables from the original matrix x. Then, using their
...
# df = 2 is equivalent to FP degree one, resulting in the generation of 8 variables.
# If acdx is set to TRUE, however, 64 variables are generated. For more
# information, see Royston (2016).

# x =  is a matrix of predictors that includes the xi variable of interest. It is
# assumed that continuous variables have already been shifted and scaled.
...
# df = Degrees of freedom in which 1 indicates linear, 2 indicates FP1, 4 indicates FP2, and so on.
  ...
  if(!all(is.na(unlist(adjustpowers,use.names = F)))){
    ...
    for(i in 1:nv){
      ...
      if(acdxa[i]){
        ...
        xadjust.T[[i]] <- afracgen(x = xadjust[,i,drop=TRUE], power = adjustpowers[[i]],
                                  scale = 1, shift = 0, s = powers)
      }else{
        xadjust.T[[i]] <- fracgen(x = xadjust[,i,drop=TRUE], power = adjustpowers[[i]],
                                  scale = 1, shift = 0)
      }
    }
