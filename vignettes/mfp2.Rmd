...
term (0), ), implying that the log function is fixed for cavol. 
...
powx <- list(age = c(1, 0.5), cavol = 0)   
# Default Interface
mfp2(x, y, criterion = "pvalue", powers = powx)

# Formula Interface
mfp2(lpsa ~ fp(age, powers = c(1, 0.5) ) + svi + fp(pgg45) + fp(cavol, powers = 0) + fp(weight) + fp(bph) +  fp(cp), data = prostate)
...
The example below demonstrates the effectiveness of MFPA in mitigating the impact of extreme covariate values on a selected function for variable `x5` in the `art` data. We see that the extreme value (observation number 151) has minimal influence on the functional form, compared to usual FP2 as shown in Figure 5.
 
```{r out.lines = 90, R.options = list(width = 100),fig.cap="Art data. Effectiveness of MFPA in mitigating the impact of extreme covariate values",fig.align='center', out.width= '45%',out.height='50%', fig.show='hold'}
# load art data
data("art")
# create matrix x and outcome y
x <- as.matrix(art[,-1])
...
fracplot(fit1, terms = "x5", color_points  = "black")[[1]] +
  ggplot2::ylab("Partial predictor + residuals")

fracplot(fit2, terms = "x5", color_points  = "black")[[1]] +
  ggplot2::ylab("Partial predictor + residuals")
