...
```{r terms_seq, eval= TRUE, fig.cap="Art data. Illustration on how to use terms_seq argument in fracplot"}
# load art data
data("art")
...
```
The selected function is FP1(1,1) which is equivalent to $\beta_0 + \beta_1x^1 +\beta_2a^1$. The function is shown below

```{r,fig.cap="Art data. Sigmoid relationship between x and outcome"}
...
```{r out.lines = 103, R.options = list(width = 100),fig.cap="Art data. Effectiveness of MFPA in mitigating the impact of extreme covariate values",fig.align='center', fig.style='page-break-inside: avoid;'}
# load art data
data("art")
# create matrix x and outcome y
x <- as.matrix(art[,-1])
...
# fit mfp using default interface with default parameters
fit <- mfp2(x,y, acdx = "x5", verbose = FALSE)
fit$fp_terms

fracplot(fit, terms = "x5")[[1]] + ggplot2::ylab("Partial predictor + residuals")
```
