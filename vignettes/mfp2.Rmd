...
Categorical variables can be measured using an **ordinal scale** or a **nominal scale**. An ordinal variable has levels that follow a meaningful order, such as 
income categories (low, medium, high). A nominal variable has categories with no intrinsic order, such as blood type or gender.  

During variable selection, some dummy variables may be eliminated, which is 
effectively equivalent to combining levels of a variable. For more details on 
handling categorical variables in regression modeling, see Royston and Sauerbrei
(2008, Chapter 3).  

In R, the `model.matrix()` function automatically generates dummy variables for factors. Nominal factors can be supplied to the formula interface and are expanded internally. Ordered factor predictors, however, are not passed directly to `mfp2.formula()`, because their polynomial contrast columns would otherwise be treated as separate MFP candidate variables. For ordinal predictors, create the intended numeric dummy or contrast columns explicitly before fitting. The `mfp2` package provides the `contr.cumulative()` function for cumulative ordinal contrasts; the example below uses equivalent cumulative indicator columns for clarity.  

Before fitting a model, decide how the ordinal information should enter the regression model. The example below represents the three-level ordinal variable `x4` by two cumulative indicators, while the nominal variable `x9` remains a standard factor.
...
x4_num <- as.integer(as.character(art$x4))
art$x4_ge_2 <- as.integer(x4_num >= 2)
art$x4_ge_3 <- as.integer(x4_num >= 3)
art$x4 <- NULL
...
# Manually create model matrix for all predictors
x <- model.matrix(y ~ . , data = art)[, -1, drop = FALSE]  # exclude intercept
head(x)
y <- as.numeric(art$y)

# Fit the model using matrix interface
