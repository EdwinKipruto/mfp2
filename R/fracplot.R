...
fracplot <- function(model,x,plotype = c("ggplot","rplot"),partials = F, col.points = "gray",
  ...
  px <- sapply(model$pwrs, function(x) all(is.na(x)))
  ...
  x.coefs <- unlist(sepcoef[if(family=="cox"){x}else{c("Intercept",x)}])
  nx <- length(sepcoef[[x]])
  ...
  kk<-if(acdx[x]){paste0(c(x,paste0("(A)",x)), ".", c(1,1))}else{paste(rep(x, nx),1:nx, sep = ".")}
  k2 <- kk[which(!is.na(pwrs[[x]]))]
  X <- model$X[,k2, drop = F]
  # calculate partial predictor: beta0 + beta^T*X or beta^T*X in case of cox
  if(family=="cox"){partial <- X%*%x.coefs}else{partial <- cbind(1,X)%*%x.coefs}
  ...
  residx <- if(family=="cox"){model$residuals}else{residuals.glm(model, type = "deviance")}
  # Component + residual.
  ...
  xx <- xx[order(xx[,x]),]
  # for title
  xpower <- unlist(model$pwrs[x], use.names = F)
  ...
  if(plotype=="rplot"){
    ...
    if(partials){
      ...
      plot(x=xx[,x],y = xx[,"partial"], xlab = xlab, ylab = ylab,type ="l",
      ...
      plot(x = xx[,x], y = xx[,"component"], xlab = xlab, pch = pch, ylab = ylab,
           type = "n",ylim = ylim,...)
    ...
    if(is.null(title)){
      ...
      title(if(acdx[x]){paste0("FP1(",paste0(xpower, collapse = ","), ")")}else{paste0("FP(",paste0(xpower, collapse = ","), ")")},line = 0.5)
    ...
    nn <- colnames(xx)
    if(partials){
      gp = ggplot(xx, aes_string(x=nn[1], y=nn[3]))  + geom_line(aes_string(x=nn[1], y=nn[2]), size = size, color = col.line)+
        geom_ribbon(aes(ymin=lower, ymax=upper), linetype=0, alpha=alpha, fill = fill)
    }else{
      gp = ggplot(xx, aes_string(x=nn[1], y=nn[3])) + geom_point(aes_string(x = nn[1], y=nn[3]), size = cex, color = col.points,shape = pch) +
        geom_ribbon(aes(ymin=lower, ymax=upper), linetype=0, alpha=alpha, fill = fill) +
        geom_line(aes_string(x=nn[1], y=nn[2]), size = size, color = col.line)
    }
   tt= if(acdx[x]){paste0("FP1(",paste0(xpower, collapse = ","), ")")}else{paste0("FP(",paste0(xpower, collapse = ","), ")")}
