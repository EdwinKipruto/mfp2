---
title: "MFP Methodology and Practical Considerations"
...
output:
  rmarkdown::html_vignette:
    ...
    highlight: tango
vignette: >
  %\VignetteIndexEntry{MFP Methodology and Practical Considerations}
...
This vignette explains the statistical motivation and methodology of
multivariable fractional polynomial modelling. It covers fractional polynomial
functions, functional-form selection, backward elimination, sample-size
considerations, influential observations, lack of fit, and selected
methodological extensions.

For practical guidance on fitting, inspecting, predicting from, and plotting
models with the `mfp2` package, see
...
The remainder of this vignette introduces fractional polynomial functions and
the multivariable fractional polynomial approach. It describes functional-form
selection for continuous covariates, variable selection, practical
considerations, and selected methodological extensions.

For guidance on fitting models with the `mfp2` package, see
`vignette("mfp2_introduction", package = "mfp2")`. Dedicated guidance is also
available for approximate cumulative distribution transformations,
spike-at-zero modelling, and MFPI interaction analysis:
...
For more comprehensive information about MFP and its extensions, see Royston
and Sauerbrei (2008), the references at the end of this vignette, and the
...
model is chosen. There are occasional exceptions. For example, to model
...
2008, Section 4.10). The complexity of the final function requires pre-definition of a
variable-selection level and a functional-form selection level, together with
the degree ($m$) of the most complex FP model allowed. In `mfp2()`, these are
called `select` and `alpha`. Typical choices are `select = alpha = 0.05` and FP2
($m = 2$). The closed test for $m = 2$ runs as follows:

1. Test the best FP2 model for $x$ against the null model using 4 d.f. and the
   `select` level. If the test is not significant, omit $x$ and stop.
2. Test the best FP2 model against a straight line using 3 d.f. and the `alpha`
   level. If the test is not significant, retain a straight line and stop.
3. Test the best FP2 model against the best FP1 model using 2 d.f. and the
   `alpha` level. Retain FP1 if this test is not significant and FP2 otherwise.
...
The multivariable procedure updates one covariate at a time while adjusting for
the current functions of all other retained covariates. The first cycle starts
from linear representations, orders covariates according to the chosen
`xorder`, and applies the variable and function selection procedure to each in
turn. Later cycles revisit every retained candidate using the functions selected
for the other covariates in the preceding updates. Iteration stops when the
selected variables, powers, and SAZ decisions no longer change, or when the
maximum number of `cycles` is reached. This cycling is essential because the
preferred form for one covariate may change after another covariate is removed
or transformed.
...
Sauerbrei (2008, Section 12.2). In `mfp2()`, the p-value procedure may instead
be replaced by direct AIC or BIC comparison of the candidate models by setting
`criterion = "aic"` or `criterion = "bic"`.

## Key issues and approaches to handling them
...
Sometimes, a continuous variable has a spike at zero: a proportion of
individuals have an observed value of zero, while the positive values follow a
continuous distribution. Common epidemiological examples include cigarette
consumption and alcohol intake.

Royston and Sauerbrei (2008, Section 4.15) and Royston et al. (2010) proposed
modelling such variables using a binary indicator that distinguishes zero from
positive values together with a fractional polynomial function for the positive
component. A two-stage procedure assesses whether the final model should retain
both components or only one of them. Becher et al. (2012) proposed a
modification of this approach.

Spike-at-zero modelling is available in `mfp2`. In the formula interface, it can
be requested with `fp(x, spike = TRUE)`. In the matrix interface, the variable
can be specified through `spike_vars`. See
`vignette("mfp2_spike", package = "mfp2")` for practical guidance.
...
## Model-selection uncertainty and limitations

MFP is a model-selection procedure. Standard errors, confidence intervals, and
p-values from the final regression fit are conditional on the selected model;
they do not include uncertainty from variable deletion, FP power selection, or
repeated backfitting. Apparent precision can therefore be optimistic, especially
when several candidate functions fit nearly equally well. Bootstrap stability
analyses, sensitivity analyses, and external validation are useful when the
selected model will support substantive conclusions or prediction.

The FP family is deliberately parsimonious and global. It may miss sharp local
features, discontinuities, complex interactions, or shapes not well represented
by low-degree FP functions. Results can be sensitive to sparse tails,
influential observations, weak signals, correlated predictors, and small
samples. MFP was not designed for high-dimensional settings in which the number
of candidate predictors is large relative to the available information. Its
selection rules should be combined with subject-matter knowledge rather than
interpreted as an automatic search for the uniquely true or predictively optimal
model.
